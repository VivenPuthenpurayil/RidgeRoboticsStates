...
public class SamplingOrderDetectorNarrow extends DogeCVDetector {
    ...
    public SamplingOrderDetectorNarrow() {
        super();
    ...
    public Mat process(Mat input) {
        ...
        if(choosenWhiteRect.get(0) != null && choosenWhiteRect.get(1) != null  && chosenYellowRect != null){
            ...
            if(leftCount == 0){
                currentOrder = SamplingOrderDetectorNarrow.GoldLocation.LEFT;
            }

            if(leftCount == 1){
                currentOrder = SamplingOrderDetectorNarrow.GoldLocation.CENTER;
            }

            if(leftCount >= 2){
                currentOrder = SamplingOrderDetectorNarrow.GoldLocation.RIGHT;
            }
            isFound = true;
            lastOrder = currentOrder;

        }
        else if(choosenWhiteRect.get(0) != null && choosenWhiteRect.get(1) != null) {
            currentOrder = GoldLocation.NOTCENTER;
        }
        else if(choosenWhiteRect.get(0) != null && chosenYellowRect != null){
            if(choosenWhiteRect.get(0).x>chosenYellowRect.x){
                currentOrder = GoldLocation.NOTRIGHT;
            }
            else{
                currentOrder = GoldLocation.NOTLEFT;
            }
        }
        else{
            currentOrder = SamplingOrderDetectorNarrow.GoldLocation.UNKNOWN;
            isFound = false;
        }
